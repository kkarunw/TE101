# TE101 Student Hub

เว็บแอปสำหรับนักศึกษารายวิชา **TE101 อุตสาหกรรมอีเว้นท์** ภาคต้น ปีการศึกษา 2569

## ข้อมูลที่แสดง
- หน้าหลักและกิจกรรมสำคัญ
- ปฏิทินการเรียน กิจกรรม Guest Speaker, IMPACT และวันสอบ
- แผนการเรียน 15 สัปดาห์
- ตารางเรียนแยก Sec 1–6
- Final Project และขั้นตอน Event Bidding
- เกณฑ์คะแนน การเข้าเรียน และวันสอบปลายภาค
- คำอธิบายรายวิชาและ CLO

> เวอร์ชันนักศึกษา **ไม่มี** ข้อมูลบริหารงบประมาณภายใน, Cash Flow, เงินสนับสนุน, ข้อมูลใบเสร็จ/หัวบิล, บันทึกส่วนตัวของผู้สอน หรือ Teaching Console

## เปิดในเครื่อง
ต้องติดตั้ง Node.js ก่อน จากนั้นเปิด Terminal ในโฟลเดอร์นี้แล้วรัน:

```bash
npm install
npm run dev
```

เปิด URL ที่ Terminal แสดง เช่น `http://localhost:5173/`

## นำขึ้น GitHub
1. สร้าง Repository ใหม่ เช่น `te101-student-hub`
2. อัปโหลดไฟล์ทั้งหมดในโฟลเดอร์นี้ขึ้น Repository โดยให้ `package.json` อยู่ที่หน้าแรกของ Repository
3. ไปที่ **Settings → Pages**
4. ที่ **Build and deployment → Source** เลือก **GitHub Actions**
5. Push เข้า branch `main`
6. เปิดแท็บ **Actions** และรอ workflow `Deploy TE101 Student Hub to GitHub Pages` ทำงานสำเร็จ
7. กลับไปที่ **Settings → Pages** จะเห็น URL ของเว็บ

ไฟล์ `.github/workflows/deploy-pages.yml` เตรียมไว้ให้แล้ว

## วันสอบ
- สอบกลางภาค: รายวิชา TE101 ไม่มีการสอบกลางภาค
- สอบปลายภาค: **3 ธันวาคม 2569 เวลา 13.00–16.00 น.**


## v1.1
- ปรับปฏิทินให้ช่องเตี้ยและกะทัดรัดขึ้น
- ปฏิทินแสดงเฉพาะหัวข้อสั้น ๆ และให้แตะวันที่เพื่อเปิด Popup รายละเอียด
- บนมือถือแสดงจุดสีแทนข้อความยาวในแต่ละวัน เพื่อไม่ให้ปฏิทินล้นจอ
- ปรับ Sidebar เป็นเมนูแนวนอนเลื่อนซ้าย–ขวาบนมือถือ
- ปรับ Layout หน้า Section, Final Project, คะแนน และข้อมูลรายวิชาให้เหมาะกับ iPad และมือถือ


## Leave request update
- เพิ่มเมนู `การลาเรียน`
- ลาป่วย: ต้องมีใบรับรองแพทย์
- ลากิจ: ต้องมีหลักฐานราชการหรือเอกสารที่มีลายเซ็นรับรอง
- การลาไปทำงาน / ไม่มีหลักฐาน / ไม่มีเหตุผลอันสมควร จะไม่รับรอง
- กรณีได้รับอนุมัติ คะแนนกิจกรรมในคาบนั้นได้รับเทียบเท่าผู้ที่เข้าเรียน
- เพิ่มลิงก์ Microsoft Forms และ QR Code สำหรับแบบฟอร์มลาเรียน


## v2.0 — Minimal UI + Course Introduction
- ปรับ UI ทั้งระบบใหม่เป็นแนวมินิมอล โทนขาว/ดำ/ม่วงอ่อน
- เพิ่มหน้า `แนะนำรายวิชา`
- เพิ่มข้อมูลผู้สอน รูปผู้สอน ประวัติการศึกษา และประสบการณ์
- เพิ่มกิจกรรมการเรียนการสอน
- เพิ่มโจทย์ Event Bidding (Midterm Project) พร้อมเกณฑ์ 10 คะแนน
- เพิ่มโจทย์ Final Project พร้อมเกณฑ์ 20 คะแนน
- รวมคำอธิบายรายวิชาและ CLO ไว้ในหน้าแนะนำรายวิชา
- ปรับ Desktop / iPad / Mobile responsive ใหม่
